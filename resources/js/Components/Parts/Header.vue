...
<header class="py-4">
  <div class="flex items-end justify-between w-full md:mb-5 pb-3 border-b-2 border-gray-500">
    <div class="w-36 sm:w-44 ml-2 sm:ml-8 shrink-0"> 
    ...
    <div class="hidden md:flex flex-wrap justify-center">
      <div class="h-7 ml-6 mb-1">
        <form :action="route('fanga')" method="GET">
          <input type="text" name="keyword" placeholder="FANZA" class="w-52 h-7 pb-2.5 leading-5 rounded-l-lg">
          <input type="submit" value="検索" class="w-10 h-7 p-1 text-gray-100 text-sm bg-gray-600 rounded-r-lg cursor-pointer">
        </form>
      </div>
      <div class="h-7 ml-6">
        <form :action="route('fanga')" method="GET">
          <input type="text" name="keyword" placeholder="DUGA" class="w-52 h-7 pb-2.5 leading-5 rounded-l-lg">
          <input type="submit" value="検索" class="w-10 h-7 p-1 text-gray-100 text-sm bg-gray-600 rounded-r-lg cursor-pointer">
        </form>
      </div>
    </div>
    ...
    <nav class="hidden md:flex items-center md:mr-2 lg:mr-8">
      <ul class="flex flex-wrap lg:flex-nowrap">
        ...
        <li class="h-6 w-16 mb-1 text-center text-gray-700 font-bold hover:bg-red-500 hover:text-white border-r-2 border-gray-500 relative">
          <Link :href="route('fanga')" class="absolute top-0 left-0 w-full h-full">BBS</Link></li>
        <li class="h-6 w-24 mb-1 text-center text-gray-700 font-bold hover:bg-red-500 hover:text-white border-r-2 border-gray-500 relative">
          <Link :href="route('fanga')" class="absolute top-0 left-0 w-full h-full">マイページ</Link></li>
        <li v-if="$page.props.auth.user" class=""></li>
        <li v-else class="h-6 w-24 mb-1 text-center text-gray-700 font-bold hover:bg-green-600 hover:text-white border-r-2 border-gray-500 relative">
          <Link :href="route('register')" class="absolute top-0 left-0 w-full h-full">会員登録</Link></li>
        <li v-if="$page.props.auth.user" class="h-6 w-24 mb-1 text-center text-gray-700 font-bold hover:bg-red-500 hover:text-white relative">
          <Link :href="route('logout')" class="absolute top-0 left-0 w-full h-full">ログアウト</Link></li>
        <li v-else class="h-6 w-20 mb-1 text-center text-gray-700 font-bold hover:bg-red-500 hover:text-white relative">
    ...
    <ul v-else class="flex">
      ...
      <li class="h-9 w-1/4 bg-gray-200 text-center text-gray-700 font-bold p-1 border-y-2 border-r-2 border-gray-500 hover:bg-red-500 hover:text-white relative">
        ...
        <Link :href="route('login')" class="absolute top-0 left-0 w-full h-full mt-1">
          <span class="inlien-block py-1 px-3 font-mono text-white bg-red-500 rounded-md">ログイン
          </span></Link></li>
    </ul>
    <div v-if="path.includes('newvideo') === true" class="mt-8">
      ...
      <div class="flex flex-nowrap md:hidden h-8 w-full mb-6 bg-gray-200 text-center font-mono text-sm">
        ...
        <Link :href="route('fanga')" class="absolute top-0 left-0 w-full h-full mt-1">ジャンル一覧</Link></div>
        <div class="h-8 w-1/4 pt-1 border-b-2 border-r-2 border-gray-500 hover:bg-red-500 hover:text-white relative">
        <Link :href="route('fanga')" class="absolute top-0 left-0 w-full h-full mt-1">女優一覧</Link></div>
        <div class="h-8 w-1/4 pt-1 border-b-2 border-r-2 border-gray-500 hover:bg-red-500 hover:text-white relative">
        <Link :href="route('fanga')" class="absolute top-0 left-0 w-full h-full mt-1">メーカー一覧</Link></div>
        <div class="h-8 w-1/4 pt-1 border-b-2 border-gray-500 hover:bg-red-500 hover:text-white relative">
        <Link :href="route('fanga')" class="absolute top-0 left-0 w-full h-full mt-1">シリーズ一覧</Link></div>
      </div>
    </div>
    <div v-if="path.includes('dugavideo') === true" class="mt-8">
      <div class="flex h-8 pl-8 min-w-full bg-gray-900 items-center">
        <h1 class="text-red-500 text-xl font-bold">DUGA</h1>
      </div>
      <div class="flex flex-nowrap md:hidden h-8 w-full mb-6 bg-gray-200 text-center font-mono text-sm">
      <div class="h-8 w-1/4 pt-1 border-b-2 border-r-2 border-gray-500 hover:bg-red-500 hover:text-white relative">
        <Link :href="route('fanga')" class="absolute top-0 left-0 w-full h-full mt-1">カテゴリ一覧</Link></div>
        <div class="h-8 w-1/4 pt-1 border-b-2 border-r-2 border-gray-500 hover:bg-red-500 hover:text-white relative">
        <Link :href="route('fanga')" class="absolute top-0 left-0 w-full h-full mt-1">女優一覧</Link></div>
        <div class="h-8 w-1/4 pt-1 border-b-2 border-r-2 border-gray-500 hover:bg-red-500 hover:text-white relative">
        <Link :href="route('fanga')" class="absolute top-0 left-0 w-full h-full mt-1">メーカー一覧</Link></div>

...
import MyPageMenu from '../Duga/MyPageMenu.vue';
...
<template>
  <section v-if="display === 'video'">
    ...
    <MyPageMenu :memoType="memoType" :lists="videolists" :display="display" :sort="sort" :group="group" :type="type"
      :hits="hits" :color="'hover:bg-blue-200'" />
    <!-- トップ -->
    <ul v-if="type === 'top'" class="flex flex-col lg:flex-row lg:flex-wrap list-none break-words">
      <li v-for="videolist in videolists.data" :key="videolist.id">
        <div class="flex flex-nowrap lg:w-[550px] lg:mr-8 mb-6 border-2 border-gray-300">
          <Link v-if="videolist.duga.duga_nice" :href="route('dnice.destroy', { productid: videolist.productid })"
          ...
          <Link v-else :href="route('dnice.store', { productid: videolist.productid, duga_id: videolist.duga_id })"
            class="flex w-10 border-r-2 border-gray-300 items-center justify-center group hover:bg-pink-300"
            preserve-scroll>
          ...
          <Link :href="route('dvideo.show', { id: videolist.productid })"
            class="flex w-full mx-auto p-2 hover:bg-blue-100 group">
          <img v-if="videolist.duga.jacketimage"
            :src="'https://pic.duga.jp/unsecure/' + videolist.re_productid + '/noauth/jacket_240.jpg'"
            :alt="'【DUGA】' + videolist.duga.title + 'のジャケット画像'" class="h-28 w-20">
          <img v-else :src="'https://pic.duga.jp/unsecure/' + videolist.re_productid + '/noauth/240x180.jpg'"
            :alt="'【DUGA】' + videolist.duga.title + 'のメイン画像'" class="h-28 w-20">
          <div class="flex flex-col w-full justify-between">
            <p class="p-1 ml-2 mb-2 font-mono bg-gray-100 group-hover:bg-white">
              {{ videolist.duga.title }}
            </p>
            <p class="pl-2 ml-auto">メモの最終更新日:
    ...
    <ul v-if="type === 'image'" class="flex flex-wrap list-none">
      <li v-for="videolist in videolists.data" :key="videolist.id" class="mr-3 mb-4">
        ...
        <img v-if="videolist.duga.jacketimage"
          :src="'https://pic.duga.jp/unsecure/' + videolist.re_productid + '/noauth/jacket_240.jpg'"
          :alt="'【DUGA】' + videolist.duga.title + 'のジャケット画像'" class="h-44 w-32">
        <img v-else :src="'https://pic.duga.jp/unsecure/' + videolist.re_productid + '/noauth/240x180.jpg'"
          :alt="'【DUGA】' + videolist.duga.title + 'のメイン画像'" class="h-44 w-32">
        </Link>
      </li>
    </ul>
    <!-- タイトル -->
      ...
      <table class="table-fixed [&>tbody>*:nth-child(even)]:bg-stone-100">
        <thead>
          <tr class="border border-gray-800 bg-gray-800 text-white">
            ...
            <th class="block w-24 md:w-40">最終更新日</th>
          </tr>
        </thead>
        <tbody>
  ...
  <section v-if="display === 'memo'">
    ...
    <MyPageMenu :memoType="memoType" :lists="memolists" :display="display" :sort="sort" :group="group" :type="type"
      :hits="hits" :color="'hover:bg-blue-200'" />
    <ul class="flex flex-col list-none">
      <li v-for="memolist in memolists.data" :key="memolist.id">
        <div class="flex py-1 items-center border-dotted border-b border-gray-500">
          <Link v-if="group"
            :href="route('mypage.' + memoType, { page: memolists.current_page, display: 'memo', sort: sort, hits: hits })"
            preserve-scroll>
          <img v-if="memolist.duga.jacketimage"
            :src="'https://pic.duga.jp/unsecure/' + memolist.re_productid + '/noauth/jacket_240.jpg'"
            :alt="'【DUGA】' + memolist.duga.title + 'のジャケット画像'" class="h-16 w-12">
          <img v-else :src="'https://pic.duga.jp/unsecure/' + memolist.re_productid + '/noauth/240x180.jpg'"
            :alt="'【DUGA】' + memolist.duga.title + 'のメイン画像'" class="h-16 w-12">
          </Link>
          <Link v-else
            :href="route('mypage.' + memoType, { page: 1, display: 'memo', group: memolist.productid, type: 'group', sort: sort, hits: hits })"
            preserve-scroll>
          <img v-if="memolist.duga.jacketimage"
            :src="'https://pic.duga.jp/unsecure/' + memolist.re_productid + '/noauth/jacket_240.jpg'"
            :alt="'【DUGA】' + memolist.duga.title + 'のジャケット画像'" class="h-16 w-12">
          ...
          <div class="w-full ml-5">
            <div class="mb-3 items-center">
              ...
              <span v-else-if="memolist.nickname === 0">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
            ...
            <div class="w-full whitespace-pre-line break-all">
              <p v-if="memolist.f_position === 'top' && memolist.f_link" class="mb-2 font-mono">>>
                No.
                <Link
                  :href="route('fmemo.memoListSort', { sort: 'oldest', content_id: memolist.f_link }) + '#f-' + memolist.f_no"
                  class="text-blue-500 hover:text-red-500 hover:underline">
                F-{{ memolist.f_no }}
                </Link>
              </p>
              <p v-if="memolist.d_position === 'top' && memolist.d_link" class="mb-3 font-mono">>>
                No.
                <Link
                  :href="route('dmemo.memoListSort', { sort: 'oldest', productid: memolist.d_link }) + '#d-' + memolist.d_no"
                  class="text-blue-500 hover:text-red-500 hover:underline">
                D-{{ memolist.d_no }}
                </Link>
              </p>
              {{ memolist.release }}
              <p v-if="memolist.f_position === 'bottom' && memolist.f_link" class="mt-3 font-mono">>> No.
                <Link
                  :href="route('fmemo.memoListSort', { sort: 'oldest', content_id: memolist.f_link }) + '#f-' + memolist.f_no"
                  class="text-blue-500 hover:text-red-500 hover:underline">
                F-{{ memolist.f_no }}
              ...
              <p v-if="memolist.d_position === 'bottom' && memolist.d_link" class="mt-2 font-mono">>> No.
                <Link
                  :href="route('dmemo.memoListSort', { sort: 'oldest', productid: memolist.d_link }) + '#d-' + memolist.d_no"
                  class="text-blue-500 hover:text-red-500 hover:underline">D-{{ memolist.d_no }}
                </Link>
              </p>
              <p class="flex text-xs text-gray-500 font-mono justify-end">
                No.
                <Link
                  :href="route('dmemo.memoListSort', { sort: 'oldest', productid: memolist.productid }) + '#d-' + memolist.id"
                  class="text-blue-500 hover:text-red-500 hover:underline">
                D-{{ memolist.id }}
                </Link>
              </p>

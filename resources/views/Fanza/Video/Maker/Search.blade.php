            ...
            <div class="h-8 ml-2">
                <form action="{{ route('flist.maker_search') }}" method="GET">
                    ...
                    <input type="submit" value="検索"
                        class="h-8 p-1 text-gray-100 text-sm bg-gray-600 rounded-r-lg cursor-pointer">
                </form>
            </div>
        </div>
        <div class="border-b-4 border-gray-500 mb-4">
            <h1 class="font-bold md:text-xl inline-block"><span class="text-red-500">【FANZA】</span>AVメーカー :『
                {{ $keyword }} 』の検索結果</h1>
            <span class="ml-5 inline-block">{{ $makersearchlists->total() }}件中
                {{ $makersearchlists->firstItem() }}〜{{ $makersearchlists->lastItem() }}件</span>
        </div>

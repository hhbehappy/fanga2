<x-app>
  ...
  <div class="border-b-4 border-gray-500 mb-2">
    <h1 class="font-bold text-xl inline-block"><span class="text-red-500">【FANZA】</span>{{ $keyword }}の動画一覧</h1>
    <span class="ml-5">{{ $videolists->total() }}件中 {{ $videolists->firstItem() }}〜{{ $videolists->lastItem() }}件</span>
  </div>
  ...
  <div class="container mx-3 flex flex-col">
    <div class="flex flex-none flex-wrap mb-4 relative">
      ...
      <a href="{{ route('fvideo.show', [ 'id' => $videolist->content_id ]) }}">
          ...
          @endif
          <img src="<?php echo 'https://pics.dmm.co.jp/digital/video/' .  $videolist->content_id  . '/' .  $videolist->content_id   . 'ps.jpg'; ?>" alt="[FANZA] <?php echo $videolist->title ?>" class="w-32 inline-block px-2 mb-4">
        </a>
      @endforeach
    </div>
    <div class="ml-6">

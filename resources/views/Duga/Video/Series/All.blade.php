<x-app>
  ...
  <section>
    ...
    <div class="border-b-4 border-gray-500 mb-4">
      <h1 class="font-semibold md:text-xl inline-block"><span class="text-red-500">【DUGA】</span>シリーズ一覧 : 全シリーズ</h1>
      <span class="ml-5 inline-block">{{ $seriesalllists->total() }}件中 {{ $seriesalllists->firstItem() }}〜{{ $seriesalllists->lastItem() }}件</span>
    </div>
    <div class="container ml-1 md:ml-0 flex flex-col">
      <div class="flex flex-wrap ml-4 md:mx-2 justify-center sm:justify-start">
        ...
        <div class="w-36 mr-5 md:mr-2 mb-4">
          <a href="{{ route('dlist.show', [ 'sort' => 'series', 'keyword' => $seriesalllist->series ]) }}" class="p-2 border-2 text-blue-500 text-center inline-block">
            @if( $seriesalllist->jacketimage )
            <img src="<?php echo $seriesalllist->jacketimage ?>" alt="【DUGA】<?php echo $seriesalllist->title ?>のメイン画像" class="h-40 w-[118px] inline-block px-1 mb-4">
            @else
            <img src="<?php echo $seriesalllist->posterimage ?>" alt="【DUGA】<?php echo $seriesalllist->title ?>のメイン画像" class="h-40 w-[118px] inline-block px-1 mb-4">
            @endif
      ...
      <div class="mt-5">
        <div class="block md:hidden">
          {{ $seriesalllists->onEachSide(0)->links('vendor.pagination.tailwind2') }}
        </div>
        <div class="md:block lg:hidden">
          {{ $seriesalllists->onEachSide(1)->links('vendor.pagination.tailwind3') }}
        </div>
        <div class="lg:block hidden">
          {{ $seriesalllists->onEachSide(2)->links('vendor.pagination.tailwind3') }}
        </div>
      </div>
    </div>
  </section>
